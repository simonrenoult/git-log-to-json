...
function gitLogToJson (dir, options) {
  ...
  const tokenizedFormat = JSON.stringify(format).replace(/"/gm, quoteOut)
  const cmd = `git -C ${dir} log ${commitLimit} --pretty=format:'${tokenizedFormat},'`
